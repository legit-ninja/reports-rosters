...
function intersoccer_render_booking_report_tab() {
    ...
    $canton = isset($_GET['canton']) ? sanitize_text_field($_GET['canton']) : '';
    ...
    <div class="reports-filters">
        <form method="get" action="">
            ...
            <div class="filter-row">
                ...
                <input type="text" id="start_date" name="start_date" value="<?php echo esc_attr($start_date); ?>" class="datepicker">

                <label for="end_date"><?php _e('End Date:', 'intersoccer-reports-rosters'); ?></label>
                <input type="text" id="end_date" name="end_date" value="<?php echo esc_attr($end_date); ?>" class="datepicker">
                ...
                <select id="venue" name="venue">
                    ...
                    <?php
                    global $wpdb;
                    $venues = $wpdb->get_results("SELECT DISTINCT t.name FROM {$wpdb->terms} t
                                                JOIN {$wpdb->term_taxonomy} tt ON t.term_id = tt.term_id
                                                WHERE tt.taxonomy = 'pa_intersoccer-venues' ORDER BY t.name");
                    foreach ($venues as $v) {
                        echo '<option value="' . esc_attr($v->name) . '" ' . selected($venue, $v->name, false) . '>' . esc_html($v->name) . '</option>';
                    }
                    ?>
                </select>

                <label for="canton"><?php _e('Canton:', 'intersoccer-reports-rosters'); ?></label>
                <select id="canton" name="canton">
                    <option value=""><?php _e('All', 'intersoccer-reports-rosters'); ?></option>
                    <?php
                    $cantons = $wpdb->get_col("SELECT DISTINCT meta_value FROM {$wpdb->prefix}woocommerce_order_itemmeta
                                             WHERE meta_key = 'Canton / Region' AND meta_value != '' ORDER BY meta_value");
                    foreach ($cantons as $c) {
                        echo '<option value="' . esc_attr($c) . '" ' . selected($canton, $c, false) . '>' . esc_html($c) . '</option>';
                    }
                    ?>
                </select>

                <input type="submit" value="<?php _e('Filter', 'intersoccer-reports-rosters'); ?>" class="button">
                <a href="<?php echo admin_url('admin.php?page=intersoccer-reports&tab=booking-report'); ?>" class="button"><?php _e('Reset', 'intersoccer-reports-rosters'); ?></a>
                <button type="button" id="export-booking-report" class="button button-primary"><?php _e('Export CSV', 'intersoccer-reports-rosters'); ?></button>
            </div>
        </form>
    </div>

    <div id="booking-report-results">
        <?php
        // Include the data processing file to get the booking report data
    ...
    jQuery(document).ready(function($) {
        ...
        $('#export-booking-report').click(function() {
            var data = {
            ...
            $.post(intersoccer_reports_ajax.ajax_url, data, function(response) {
                if (response.success) {
