...
require_once dirname(__FILE__) . '/final-reports-aggregation.php';
 ...
 * @param bool        $exclude_buyclub When true, omit BuyClub (zero-net lines and matching coupon codes).
 * @param bool        $live When true, include processing orders (Live Snapshot mode).
 * @return array Report data grouped by date range, canton, venue, and camp type
 */
function intersoccer_get_final_reports_data($year, $activity_type, $season_type = null, $region = null, $exclude_buyclub = false, $live = false) {
    global $wpdb;
    ...
    if ($activity_type === 'Camp') {
        ...
        // Region filter applied in PHP after canton enrichment.
        ...
        foreach ($rosters as $idx => &$roster) {
            if (function_exists('intersoccer_reports_enrich_camp_canton_from_product')) {
                intersoccer_reports_enrich_camp_canton_from_product($roster);
            }
            $canton_disp = isset($roster['canton']) ? trim((string) $roster['canton']) : '';
            if (($canton_disp === '' || strcasecmp($canton_disp, 'Unknown') === 0) && !empty($roster['region'])) {
                $roster['canton'] = trim((string) $roster['region']);
            }
            ...
            if (!empty($roster['from_rosters_table'])) {
                ...
                if (empty($roster['event_start_date']) || $roster['event_start_date'] === '1970-01-01') {
                    ...
                    // Do not fabricate placeholder dates (e.g. Jan 1); skip rows until real dates exist.
                    ...
                    if (!empty($event_start_date) && $event_start_date !== '1970-01-01') {
                        ...
                        if ($season_year_undated !== null && $season_year_undated == $requested_year_int) {
                            ...
                            if ($label !== '' && $label !== 'N/A' && function_exists('intersoccer_reports_normalize_camp_terms_for_dates')) {
                                $label = intersoccer_reports_normalize_camp_terms_for_dates($label);
                            }
                            if ($label === '' || $label === 'N/A') {
                ...
                if (!empty($region) && function_exists('intersoccer_reports_region_matches_filter')) {
                    ...
                    if (!intersoccer_reports_region_matches_filter($roster_canton, $region)) {
                        $skipped_year_mismatch++;
                        continue;
                    }
                }
            ...
            if (!empty($camp_terms) && $camp_terms !== 'N/A' && function_exists('intersoccer_reports_normalize_camp_terms_for_dates')) {
                $camp_terms = intersoccer_reports_normalize_camp_terms_for_dates($camp_terms);
                $roster['camp_terms'] = $camp_terms;
            }
                    ...
                    if ($roster_data && !empty($roster_data['start_date'])) {
                        ...
                        if ($season_year_undated !== null && $season_year_undated == intval($year)) {
                            ...
                            $roster['undated_group_label'] = !empty($camp_terms) && $camp_terms !== 'N/A'
                                ? (function_exists('intersoccer_reports_normalize_camp_terms_for_dates')
                                    ? intersoccer_reports_normalize_camp_terms_for_dates(trim((string) $camp_terms))
        ...
        foreach ($rosters as &$roster) {
            if (function_exists('intersoccer_reports_enrich_course_venue_and_day')) {
                intersoccer_reports_enrich_course_venue_and_day($roster);
            }
            // BuyClub: orders with original price > 0 and final price = 0
            // Now using data from main query (line_subtotal and line_total already fetched)
            $line_subtotal = floatval($roster['line_subtotal'] ?? 0);
        ...
        if (!empty($region) && function_exists('intersoccer_reports_region_matches_filter')) {
            $rosters = array_values(array_filter($rosters, static function ($row) use ($region) {
                return intersoccer_reports_region_matches_filter($row['canton'] ?? '', $region);
            }));
        }

        return intersoccer_reports_build_course_report_from_entries($rosters, $exclude_buyclub);
    }
}
                ...
                foreach ($venues as $venue => $camp_types) {
                    foreach ($camp_types as $camp_type => $data) {
                        ...
                        if ($camp_type === 'Full Day') {
                            ...
                            $bc = (int) ($data['buyclub'] ?? 0);
                            $totals['full_day']['online'] = $totals['full_day']['full_week'] + $bc;
                            $totals['full_day']['total'] += (int) $data['full_week'] + $bc;
                            ...
                            $bc = (int) ($data['buyclub'] ?? 0);
                            $totals['mini']['online'] = $totals['mini']['full_week'] + $bc;
                            $totals['mini']['total'] += (int) $data['full_week'] + $bc;
                            $totals['mini']['unique_records'] += $unique_records;
                        }

                        // All camps total (All registrations style: FW + BuyClub)
                        $bc = (int) ($data['buyclub'] ?? 0);
                        $totals['all']['full_week'] += $data['full_week'];
                        foreach ($data['individual_days'] as $day => $count) {
                            $totals['all']['individual_days'][$day] += $count;
                        }
                        $totals['all']['online'] = $totals['all']['full_week'] + $bc;
                        $totals['all']['total'] += (int) $data['full_week'] + $bc;
