...
if (!function_exists('intersoccer_reports_aggregate_camp_location_group')) {
     ...
     * @return array{full_week:int,individual_days:array<string,int>,min_max:string,unique_records:int}
     */
...
if (!function_exists('intersoccer_reports_resolve_facet_label')) {
    ...
    function intersoccer_reports_resolve_facet_label($raw, $taxonomy, $fallback = 'Unknown') {
        ...
        if (function_exists('intersoccer_roster_facet_for_grouping')) {
            $group = intersoccer_roster_facet_for_grouping($raw, $taxonomy);
            if ($group !== '') {
                return $group;
            }
        } elseif (function_exists('intersoccer_get_term_name') && $raw !== '') {
...
if (!function_exists('intersoccer_reports_camp_excel_data_rows')) {
     ...
     * @return array<int,array{0:string,1:string,2:string,3:string,4:int,5:int,6:int,7:int,8:int,9:int,10:string,11:int}>
     */
    function intersoccer_reports_camp_excel_data_rows(array $report_data) {
        $rows = [];
                ...
                foreach ($venues as $venue => $camp_types) {
                    ...
                    foreach ($camp_types as $camp_type => $data) {
                        ...
                        $rows[] = [
                            ...
                            (int) ($days['Friday'] ?? 0),
                            (string) ($data['min_max'] ?? ''),
                            (int) $data['full_week'] + array_sum(array_map('intval', $days)),
                        ];
                    }
                }
            }
