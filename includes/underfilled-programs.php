...
function intersoccer_render_underfilled_programs_page() {
    ...
    $activity_filter = isset($_GET['activity']) ? sanitize_text_field($_GET['activity']) : 'both';
    if (!in_array($activity_filter, ['both', 'camp', 'course'], true)) {
        $activity_filter = 'both';
    }
    $season_type = isset($_GET['season_type']) ? sanitize_text_field($_GET['season_type']) : 'Summer';
    ...
    // CSV export
    if (!empty($_GET['export_csv']) && check_admin_referer('intersoccer_underfilled_csv')) {
        nocache_headers();
        header('Content-Type: text/csv; charset=utf-8');
        header('Content-Disposition: attachment; filename=underfilled-programs-' . $year . '.csv');
        $out = fopen('php://output', 'w');
        fputcsv($out, ['Activity', 'Product', 'Venue', 'Season', 'Term/Day', 'Age Group', 'Players', 'Urgency']);
        foreach ($rows as $row) {
    ...
    <div class="wrap intersoccer-underfilled-programs">
        ...
        <form method="get" action="<?php echo esc_url(admin_url('admin.php')); ?>" style="margin-bottom:16px;">
            ...
            <a class="button" href="<?php echo esc_url($csv_url); ?>"><?php esc_html_e('Export CSV', 'intersoccer-reports-rosters'); ?></a>
